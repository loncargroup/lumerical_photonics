...
def generate_cavity_and_check(pcc_params):
    ...
    flag = False
    if a_mirr-hx_mirr < min_dim or hx_mirr < min_dim or (w0 - hy_mirr)/2 < min_dim or hy_mirr < min_dim \
            or (a_def-hx_def) < min_dim or hx_def < min_dim or (w0 - hy_def)/2 < min_dim or hy_def < min_dim:
...
def fitness(cavity_params):
    ...
    if flag:
        ...
        r1 = cavity.simulate("resonance", target_freq=source_frequency, source_pulselength=60e-15,
                              analyze_time=600e-15, mesh_regions=[man_mesh], sim_size=Vec3(1.25, 4, 12))
...
log_name = f"C:/Users/LoncarLab/Documents/GitHub/lumerical_photonics/hole_cavity_optimization/hole_cav_opt_80nm_wHy_0.txt"

lat_const = .27
p0 = np.array([.173, .683, 1.4])
# dA, cX, cY
bounds = ((.05, .25),  (.5, .9), (1, 1.7))
